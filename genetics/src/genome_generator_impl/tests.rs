...
fn register_cluster_gene_selection_expectations(
    ...
    for &selection in cluster_gene_selections {
        ...
        random
            .expect_flip_coin_with_probability(|arg| {
                arg.partial_eq(PROBABILITY_FOR_NEW_CLUSTER_GENE)
            })
        ...
        if let DetailedClusterGeneSelection::Existing(cluster_gene_index) = selection {
            random
                .expect_usize_in_range(
                    |arg| arg.partial_eq(0),
                    |arg| arg.partial_eq(generated_cluster_gene_count),
                )
